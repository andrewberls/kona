...
Kona.Keys = {
  ...
  bind: function(key, handler) {
    ...
    if (!(Kona.Keys._handlers[key] != null)) {
      Kona.debug("  key " + key + " not found in existing handlers, setting to []\n");
      Kona.Keys._handlers[key] = [];
    }
    return Kona.Keys._handlers[key].push(handler);
  },
  dispatch: function(event) {
    var handler, key, _i, _len, _ref, _results;
    key = event.keyCode;
    Kona.debug("\ndispatching key: " + key);
    if (!(Kona.Keys._handlers[key] != null)) {
      Kona.debug("  no handler found, ABORT");
    }
    if (Kona.Keys.reject(event) || !(Kona.Keys._handlers[key] != null)) {
      return;
    }
    _ref = Kona.Keys._handlers[key];
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (event.cancelBubble) {
        _results.push(event.cancelBubble = true);
      } else {
...
Kona.ready(function() {
  return Kona.Keys.addEvent(document, 'keydown', Kona.Keys.dispatch);
});
