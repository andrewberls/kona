...
Kona.Utils = {
  inspect: function(obj, label) {
    ...
    if (label != null) {
      ...
      Kona.debug("Dumping " + (is_array ? 'array' : typeof obj) + ": " + defined_str);
    }
    if (is_array) {
      return Kona.debug("" + spacer + "[" + obj + "]");
    } else if (is_string) {
      return Kona.debug("" + spacer + "\"" + obj + "\"");
    } else if (is_number) {
      return Kona.debug("" + spacer + obj);
      ...
      for (key in obj) {
        ...
        if (value instanceof Function) {
          _results.push(Kona.debug("" + spacer + key + "(): <function>"));
        } else {
          _results.push(Kona.debug("" + spacer + key + ": " + value));
        }
      }
      return _results;
    }
